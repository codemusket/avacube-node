...
```sh
npm install git+ssh://git@github.com:codemusket/avacube-node.git
```
...
await client.smartAccountAddress.retrieve({ owner: 'owner' }, {
  maxRetries: 5,
});
```
...
const response = await client.smartAccountAddress.retrieve({ owner: 'owner' }).asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: addressResp, response: raw } = await client.smartAccountAddress
...
This can also be useful if you are getting the wrong TypeScript types for `Response` ([more details](https://github.com/codemusket/avacube-node/tree/main/src/_shims#readme)).
...
// Override per-request:
await client.smartAccountAddress.retrieve(
  { owner: 'owner' },
  {
...
We are keen for your feedback; please open an [issue](https://www.github.com/codemusket/avacube-node/issues) with questions, bugs, or suggestions.
